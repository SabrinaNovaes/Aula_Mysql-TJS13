...
CREATE TABLE IF NOT EXISTS `db_loja_games`.`tb_usuarios` (
  ...
  PRIMARY KEY (`id`, `nome_usuario`, `usuario`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `db_loja_games`.`tb_produtos` (
  ...
  `tb_usuarios_nome_usuario` VARCHAR(255) NOT NULL,
  `tb_usuarios_usuario` VARCHAR(255) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_tb_produtos_tb_categorias_idx` (`tb_categorias_id` ASC) VISIBLE,
  INDEX `fk_tb_produtos_tb_usuarios1_idx` (`tb_usuarios_id` ASC, `tb_usuarios_nome_usuario` ASC, `tb_usuarios_usuario` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_tb_produtos_tb_usuarios1`
    FOREIGN KEY (`tb_usuarios_id` , `tb_usuarios_nome_usuario` , `tb_usuarios_usuario`)
    REFERENCES `db_loja_games`.`tb_usuarios` (`id` , `nome_usuario` , `usuario`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
